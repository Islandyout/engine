...
declare const createEditorRuntime: () => Runtime;
...
function syncRuntime() {
  ...
  for (const entity of doc.scene.eachAlive()) {
    ...
    const v = doc.scene.get(entity, "Velocity")?.value ?? { x: 0, y: 0, z: 0 };
...
function updatePanels() {
  ...
  for (const entity of doc.scene.eachAlive()) {
    ...
    button.textContent = (doc.scene.has(entity, "Parent") ? "↳ " : "□ ") + name;
  ...
  parent.onchange = () =>
    execute({
      ...
      parent:
        parent.value === ""
          ? null
          : doc.scene.eachAlive().find((e) => e.index === Number(parent.value)),
    });
