...
struct GamepadConnectionEvent final {
    InputDeviceId device{};
...
using InputEvent = std::variant<KeyEvent, TextInputEvent, MouseMotionEvent, MouseButtonEvent, MouseWheelEvent,
                                TouchEvent, GamepadConnectionEvent, GamepadButtonEvent, GamepadAxisEvent>;
...
struct GamepadState final {
    ...
    std::array<bool, static_cast<usize>(GamepadButton::count)> buttons_pressed{};
    std::array<bool, static_cast<usize>(GamepadButton::count)> buttons_released{};
};
...
public:
    ...
    [[nodiscard]] bool gamepad_down(GamepadButton button,
                                    std::optional<InputDeviceId> device = std::nullopt) const noexcept;
    [[nodiscard]] bool gamepad_pressed(GamepadButton button,
                                       std::optional<InputDeviceId> device = std::nullopt) const noexcept;
    [[nodiscard]] bool gamepad_released(GamepadButton button,
                                        std::optional<InputDeviceId> device = std::nullopt) const noexcept;
    [[nodiscard]] f32 gamepad_axis(GamepadAxis axis, std::optional<InputDeviceId> device = std::nullopt) const noexcept;
...
public:
    explicit InputReplay(std::vector<InputReplayFrame> frames);

    void restart(InputState& state) noexcept;
    [[nodiscard]] bool inject_next(InputState& state);
    [[nodiscard]] u64 next_frame() const noexcept { return next_frame_; }
    [[nodiscard]] bool finished() const noexcept { return next_record_ == frames_.size(); }
    [[nodiscard]] const std::vector<InputReplayFrame>& frames() const noexcept { return frames_; }

private:
    std::vector<InputReplayFrame> frames_;
    usize next_record_{};
    u64 next_frame_{};
};
